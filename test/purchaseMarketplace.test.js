...
const PurchaseMarketplace = artifacts.require("PurchaseMarketplace")
...
contract("PurchaseMarketplace", accounts => {

  const purchaseId = "0x00000000000000000000000000003130"
  const proof = "0x0000000000000000000000000000313000000000000000000000000000003130"

  const purchaseId2 = "0x00000000000000000000000000002130"
  ...
  before(async () => {
    _contract = await PurchaseMarketplace.deployed()
    contractOwner = accounts[0]
    buyer = accounts[1]
  })

  describe("Purchase the new purchase", () => {

    before(async() => {
      await _contract.purchasePurchase(purchaseId, proof, {
        from: buyer,
        value
      })
    })

    it("should NOT allow to repurchase already owned purchase", async () => {
      await catchRevert(_contract.purchasePurchase(purchaseId, proof, {
        from: buyer,
        value
      }))
    })

    it("can get the purchased purchase hash by index", async () => {
      const index = 0
      purchaseHash = await _contract.getPurchaseHashAtIndex(index)
      const expectedHash = web3.utils.soliditySha3(
        { type: "bytes16", value: purchaseId },
        { type: "address", value: buyer },
      )

      assert.equal(purchaseHash, expectedHash, "Purchase hash is not maching the hash of purchased purchase!")
    })

    it("should match the data of the purchase purchased by buyer", async () => {
      const exptectedIndex = 0
      const exptectedState = 0
      const purchase = await _contract.getPurchaseByHash(purchaseHash)

      assert.equal(purchase.id, exptectedIndex, "Purchase index should be 0!")
      assert.equal(purchase.price, value, `Purchase price should be ${value}!`)
      assert.equal(purchase.proof, proof, `Purchase proof should be ${proof}!`)
      assert.equal(purchase.owner, buyer, `Purchase buyer should be ${buyer}!`)
      assert.equal(purchase.state, exptectedState, `Purchase state should be ${exptectedState}!`)
    })
  })

  describe("Activate the purchased purchase", () => {

    it("should NOT be able to activate purchase by NOT contract owner", async () => {
      await catchRevert(_contract.activatePurchase(purchaseHash, {from: buyer}))
    })

    it("should have 'activated' state", async () => {
      await _contract.activatePurchase(purchaseHash, {from: contractOwner})
      const purchase = await _contract.getPurchaseByHash(purchaseHash)
      const exptectedState = 1

      assert.equal(purchase.state, exptectedState, "Purchase should have 'activated' state")
    })
  })
  ...
  describe("Deactivate purchase", () => {
    let purchaseHash2 = null
    let currentOwner = null

    before(async () => {
      await _contract.purchasePurchase(purchaseId2, proof2, {from: buyer, value})
      purchaseHash2 = await _contract.getPurchaseHashAtIndex(1)
      currentOwner = await _contract.getContractOwner()
    })

    it("should NOT be able to deactivate the purchase by NOT contract owner", async () => {
      await catchRevert(_contract.deactivatePurchase(purchaseHash2, {from: buyer}))
    })
    ...
    it("should have status of deactivated and price 0", async () => {
      ...
      const result = await _contract.deactivatePurchase(purchaseHash2, {from: contractOwner})
      ...
      const purchase = await _contract.getPurchaseByHash(purchaseHash2)
      const exptectedState = 2
      const exptectedPrice = 0
      const gas = await getGas(result)

      assert.equal(purchase.state, exptectedState, "Purchase is NOT deactivated!")
      assert.equal(purchase.price, exptectedPrice, "Purchase price is not 0!")
    ...
    it("should NOT be able activate deactivated purchase", async () => {
      await catchRevert(_contract.activatePurchase(purchaseHash2, {from: contractOwner}))
    })
  })

  describe("Repurchase purchase", () => {
    let purchaseHash2 = null
    ...
    it("should NOT repurchase when the purchase doesn't exist", async () => {
      const notExistingHash = "0x5ceb3f8075c3dbb5d490c8d1e6c950302ed065e1a9031750ad2c6513069e3fc3"
      await catchRevert(_contract.repurchasePurchase(notExistingHash, {from: buyer}))
    })

    it("should NOT repurchase with NOT purchase owner", async () => {
      const notOwnerAddress = accounts[2]
      await catchRevert(_contract.repurchasePurchase(purchaseHash2, {from: notOwnerAddress}))
    })
    ...
    it("should be able repurchase with the original buyer", async () => {
      ...
      const result = await _contract.repurchasePurchase(purchaseHash2, {from: buyer, value})
      ...
      assert.equal(purchase.state, exptectedState, "The purchase is not in purchased state")
      assert.equal(purchase.price, value, `The purchase price is not equal to ${value}`)
    ...
    it("should NOT be able to repurchase purchased purchase", async () => {
      await catchRevert(_contract.repurchasePurchase(purchaseHash2, {from: buyer}))
    })
  })
